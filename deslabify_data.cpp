...
int main(int argc, char** argv) {
  ...
  vector<int16_t> data(1);
  while (datafile.read(reinterpret_cast<char*>(&data.back()),sizeof(int16_t))) {
  ...
  for (auto it=cgeom.transforms.begin();it!=cgeom.transforms.end();++it) {
    for (size_t ss=it->min_ss;ss<=it->max_ss;++ss) {
      for (size_t fs=it->min_fs;fs<=it->max_fs;++fs) {
        const double value = data[1552*ss+fs];
        cout.write(reinterpret_cast<const char*>(&value),sizeof(double));
      }
    }
  }
}
